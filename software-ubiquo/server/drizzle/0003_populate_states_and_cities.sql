-- Migration 0003: Populate state and city tables with Brazilian data
-- This migration cleans up existing data and populates with complete dataset
-- 
-- To run this migration manually:
--   PGPASSWORD=changeme psql -h localhost -U postgres -d ubiqua_db -f drizzle/0003_populate_states_and_cities.sql

-- Step 1: Clean up existing data (respecting foreign key constraints)
-- Delete cities first (child table)
TRUNCATE TABLE city CASCADE;

-- Delete states
TRUNCATE TABLE state CASCADE;

-- Step 2: Populate state table with all Brazilian states (27 states)
...
(29,'BA','Bahia',-11.81,-41.5,'Nordeste'),
...
(51,'MT','Mato Grosso',-12.64,-56.0096,'Centro-Oeste'),
(52,'GO','Goiás',-15.827,-49.8362,'Centro-Oeste'),
(53,'DF','Distrito Federal',-15.83,-47.86,'Centro-Oeste');

-- Step 3: Import all Brazilian cities (5570 cities)
-- Note: For brevity, the city data is stored in states_data/city.sql
-- This script will be executed by the seed script
\echo 'Loading cities from states_data/city.sql...'
